...
function equalSequence(haystack, start, needle) {
...
function anchorRange(edit) {
    if (edit.kind === "replace" || edit.kind === "delete")
        return edit.range;
    if (edit.at.kind === "before" || edit.at.kind === "after")
        return { start: edit.at.line, end: edit.at.line };
    return null;
}
function remapEdit(edit, mapped) {
    if (edit.kind === "replace") {
        if (!mapped)
            throw new Error("Missing rebase mapping for replacement.");
        return { ...edit, range: mapped };
    }
    if (edit.kind === "delete") {
        if (!mapped)
            throw new Error("Missing rebase mapping for deletion.");
        return { ...edit, range: mapped };
    }
    if (edit.at.kind === "before" || edit.at.kind === "after") {
        if (!mapped)
            throw new Error("Missing rebase mapping for insertion anchor.");
        return { ...edit, at: { ...edit.at, line: mapped.start } };
    }
    return edit;
}
export function rebaseEdits(previousText, currentText, edits) {
    ...
    if (!offsets.every(offset => offset === first))
        return null;
    return {
        edits: mapped,
        offset: first,
