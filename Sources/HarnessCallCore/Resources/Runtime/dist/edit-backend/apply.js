...
function assertRange(range, lineCount) {
    ...
    if (range.end > lineCount)
        throw new Error(`Line range ${range.start}..${range.end} exceeds file length ${lineCount}`);
}
export async function concretizeEdits(path, snapshotText, edits, blockResolver) {
    ...
    for (const edit of edits) {
        ...
        if (!blockResolver)
            throw new Error(`Block edit requested for ${path}, but no block resolver is configured.`);
        const span = await blockResolver({ path, text: snapshotText, line: edit.line });
        if (!span)
            throw new Error(`Could not resolve syntactic block beginning at line ${edit.line} in ${path}.`);
...
export function requiredSeenRanges(edits) {
    const out = [];
    for (const edit of edits) {
        if (edit.kind === "replace" || edit.kind === "delete")
            out.push(edit.range);
        else if (edit.at.kind === "before" || edit.at.kind === "after")
            out.push({ start: edit.at.line, end: edit.at.line });
    }
    return out;
}
function toSplice(edit, lineCount) {
...
export function applyConcreteEdits(text, edits) {
    ...
    for (let i = 0; i < splices.length; i++) {
        for (let j = i + 1; j < splices.length; j++) {
            if (overlaps(splices[i], splices[j]))
                throw new Error("Overlapping or ambiguous edits in one file are not allowed.");
        }
    }
    splices.sort((a, b) => b.start - a.start || b.end - a.end);
    const output = [...lines];
    for (const splice of splices)
        output.splice(splice.start, splice.end - splice.start, ...splice.replacement);
