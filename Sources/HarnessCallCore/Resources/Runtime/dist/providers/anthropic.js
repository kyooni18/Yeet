...
export class AnthropicProvider {
    ...
    async complete(request) {
        const response = await providerFetch(`${this.#baseUrl}/v1/messages`, { method: "POST", headers: this.#headers(), body: JSON.stringify(requestBody(request, false)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
        ...
        const toolCalls = (raw.content ?? [])
            ...
            .map((b, index) => normalizeToolCall(b.id, b.name, b.input, index));
    ...
    async *stream(request) {
        const response = await providerFetch(`${this.#baseUrl}/v1/messages`, { method: "POST", headers: this.#headers(), body: JSON.stringify(requestBody(request, true)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
            ...(request.timeoutMs !== undefined ? { timeoutMs: request.timeoutMs } : {}),
            ...(request.retry ? { retry: request.retry } : {}),
        ...
        for await (const event of parseSSE(response)) {
            ...
            if (raw.type === "content_block_start" && raw.content_block?.type === "tool_use") {
                ...
                if (raw.delta.text)
                    yield { type: "text-delta", delta: raw.delta.text };
            }
