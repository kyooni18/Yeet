...
function functionNameForToolResult(message) {
    if (message.name)
        return message.name;
...
function geminiFinish(value, hasTools) {
    if (hasTools)
        return "tool_call";
    if (value === "STOP")
        return "stop";
    if (value === "MAX_TOKENS")
        return "length";
    if (["SAFETY", "RECITATION", "PROHIBITED_CONTENT", "BLOCKLIST"].includes(String(value)))
        return "content_filter";
    return normalizeFinishReason(value);
}
function normalizeGeminiTool(part, index) {
...
export class GeminiProvider {
    ...
    #headers() {
        ...
        if (!this.#apiKey)
            throw new Error("Missing API key or OAuth token for gemini");
    ...
    async listModels() {
        ...
        do {
            ...
            const response = await providerFetch(url, { method: "GET", headers: this.#headers() }, {
                provider: this.id,
    ...
    async complete(request) {
        const response = await providerFetch(`${this.#baseUrl}/models/${encodeURIComponent(this.#modelPath(request.model))}:generateContent`, { method: "POST", headers: this.#headers(), body: JSON.stringify(bodyFor(request)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
        ...
        const toolCalls = parts
            ...
            .map((part, index) => normalizeGeminiTool(part, index));
        const normalizedUsage = usage(raw.usageMetadata?.promptTokenCount, raw.usageMetadata?.candidatesTokenCount, raw.usageMetadata?.totalTokenCount, raw.usageMetadata?.cachedContentTokenCount);
        return {
            provider: this.id,
            model: request.model,
    ...
    async *stream(request) {
        const response = await providerFetch(`${this.#baseUrl}/models/${encodeURIComponent(this.#modelPath(request.model))}:streamGenerateContent?alt=sse`, { method: "POST", headers: this.#headers(), body: JSON.stringify(bodyFor(request)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
            ...(request.timeoutMs !== undefined ? { timeoutMs: request.timeoutMs } : {}),
            ...(request.retry ? { retry: request.retry } : {}),
        ...
        for await (const event of parseSSE(response)) {
            ...
            if (raw.usageMetadata) {
                finalUsage = usage(raw.usageMetadata.promptTokenCount, raw.usageMetadata.candidatesTokenCount, raw.usageMetadata.totalTokenCount, raw.usageMetadata.cachedContentTokenCount);
            }
            if (!candidate)
                continue;
            const parts = candidate.content?.parts ?? [];
            ...
            if (candidate.finishReason)
                finish = geminiFinish(candidate.finishReason, hasTools || finish === "tool_call");
            else if (hasTools)
                finish = "tool_call";
        }
