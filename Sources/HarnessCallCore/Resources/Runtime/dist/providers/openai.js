...
function bodyFor(request, stream) {
...
function finishReason(raw) {
    const hasToolCall = (raw.output ?? []).some((item) => item.type === "function_call");
    if (hasToolCall)
        return "tool_call";
    if (raw.status === "completed")
        return "stop";
    if (raw.status === "incomplete")
        return normalizeFinishReason(raw.incomplete_details?.reason);
    return "unknown";
}
...
export class OpenAIProvider {
    ...
    async listModels() {
        const response = await providerFetch(`${this.#baseUrl}/models`, { method: "GET", headers: this.#headers() }, {
            provider: this.id,
    ...
    async complete(request) {
        const response = await providerFetch(`${this.#baseUrl}/responses`, { method: "POST", headers: this.#headers(), body: JSON.stringify(bodyFor(request, false)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
        ...
        const toolCalls = (raw.output ?? [])
            ...
            .map((item, index) => normalizeToolCall(item.call_id ?? item.id, item.name, safeJsonParse(item.arguments ?? ""), index));
        const normalizedUsage = usage(raw.usage?.input_tokens, raw.usage?.output_tokens, raw.usage?.total_tokens, raw.usage?.input_tokens_details?.cached_tokens);
        return {
            provider: this.id,
    ...
    async *stream(request) {
        const response = await providerFetch(`${this.#baseUrl}/responses`, { method: "POST", headers: this.#headers(), body: JSON.stringify(bodyFor(request, true)) }, {
            provider: this.id,
            ...(this.#fetch ? { fetch: this.#fetch } : {}),
            ...(request.timeoutMs !== undefined ? { timeoutMs: request.timeoutMs } : {}),
            ...(request.retry ? { retry: request.retry } : {}),
        ...
        for await (const event of parseSSE(response)) {
            ...
            if (raw.type === "response.output_text.delta" && typeof raw.delta === "string") {
                ...
                if (raw.item.call_id ?? raw.item.id)
                    current.id = raw.item.call_id ?? raw.item.id;
                if (raw.item.name)
                    current.name = raw.item.name;
                ...
                tools.set(index, current);
                if (raw.delta)
                    yield { type: "tool-call-delta", index, argumentsDelta: raw.delta };
            }
                ...
                if (typeof raw.arguments === "string")
                    current.argumentsText = raw.arguments;
                tools.set(index, current);
            }
        ...
        const normalizedUsage = completedRaw
            ? usage(completedRaw.usage?.input_tokens, completedRaw.usage?.output_tokens, completedRaw.usage?.total_tokens, completedRaw.usage?.input_tokens_details?.cached_tokens)
            : undefined;
        yield {
            type: "finish",
