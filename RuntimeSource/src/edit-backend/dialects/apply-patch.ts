...
function stripPrefix(file: string): string {
...
export class ApplyPatchDialect implements EditDialect {
  ...
  parse(input: string, context: { snapshots: Readonly<Record<string, string>> }): ApplyRequest {
    ...
    while (index < lines.length) {
      ...
      if (!plus?.startsWith("+++ ")) throw new Error("Malformed unified diff: missing +++ header.");
      ...
      while (index < lines.length && !lines[index]!.startsWith("--- ")) {
        ...
        index++;
        const hunk: Hunk = { oldStart, oldCount, oldLines: [], newLines: [] };
        while (index < lines.length && !lines[index]!.startsWith("@@ ") && !lines[index]!.startsWith("--- ")) {
          const row = lines[index]!;
          if (row.startsWith(" ")) {
            hunk.oldLines.push(row.slice(1));
            hunk.newLines.push(row.slice(1));
          } else if (row.startsWith("-")) hunk.oldLines.push(row.slice(1));
    ...
    if (changes.length === 0) throw new Error("No unified diff file headers were found.");
    return { changes };
  }
}
