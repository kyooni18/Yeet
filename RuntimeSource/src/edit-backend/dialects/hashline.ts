...
function consumeBody(lines: string[], start: number): { text: string; next: number } {
  const body: string[] = [];
  let index = start;
  while (index < lines.length) {
    const line = lines[index]!;
    if (line.trim() === "" || SECTION.test(line) || /^(?:PUT|CUT|REM|MV)\b/i.test(line)) break;
    if (!line.startsWith("+")) throw new Error(`Hashline body row must start with '+': ${line}`);
    body.push(line.slice(1));
    index++;
  }
  if (body.length === 0) throw new Error("PUT requires at least one '+' body row.");
...
export class HashlineDialect implements EditDialect {
  ...
  parse(input: string): ApplyRequest {
    ...
    while (index < lines.length) {
      ...
      if (!current) throw new Error("Hashline payload must begin with [path@snapshot].");
      ...
      if (/^REM\s*$/i.test(line)) {
        current.fileOp = { kind: "delete" } satisfies FileOperation;
        index++;
        continue;
      }
      if ((match = MOVE.exec(line))) {
        current.fileOp = { kind: "move", destination: unquote(match[1]!) } satisfies FileOperation;
        index++;
        continue;
      }
    ...
    pushCurrent();
    if (changes.length === 0) throw new Error("Hashline payload contained no file sections.");
    return { changes };
  }
}
